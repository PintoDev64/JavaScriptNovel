export default function LexerCallExpression(ParserNode: NLexer.INode): NLexer.INode {
    ...
    if (ParserNode.name === "Character") {
        ...
        if (ParserNode.arguments[0].type !== "StringLiteral") throw new Error(``);
        if (ParserNode.arguments[1].type !== "ArrayExpression") throw new Error(``);
        if (ParserNode.arguments[2].type !== "StringLiteral") throw new Error(``);
    }
    ...
    if (ParserNode.name === "MediaAudio") {
        ...
        if (ParserNode.arguments[0].type !== "StringLiteral") throw new Error(``);
        if (ParserNode.arguments[1].type !== "StringLiteral") throw new Error(``);
    }
    ...
    if (ParserNode.name === "Play") {
        if (ParserNode.arguments === undefined) throw new Error(`No hay argumetos en ${ParserNode.value}`);
        if (ParserNode.arguments?.length > 1) throw new Error(`La cantidad de argumentos no es la esperada. Se esperaban ${ParserNode.arguments.length}`);
        if (ParserNode.arguments[0].type !== "VariableExpression") throw new Error(``);
    }
